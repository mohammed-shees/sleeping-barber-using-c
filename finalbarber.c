...
#define MAX_CUSTOMERS 50 // Function prototypes...
...
// waitingRoom Limits the number of customers allowed  to enter the waiting room at once.
sem_t waitingRoom; 
...
int main(int argc, char *argv[]) 
{
    ...
    printf("Maximum number of customers can only be 50. Enter number of customers and chairs.\n");
    scanf("%d",&x);
    numCustomers = x; 
    scanf("%d",&x);
    numChairs = x;
  ...
  while (!allDone) { // Sleep until someone arrives and wakes you.. 
    ...
    { // Take a random amount of time to cut the customer's hair.
     ...
     sem_post(&seatBelt); 
         ...
         printf("The barb0er is going home for the day.\n"); 
    } 
   }
}
